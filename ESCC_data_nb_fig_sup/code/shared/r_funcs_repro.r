# Reproducibility helper: sourced by repaired supplementary notebooks.
# Based on stage2/scripts/r_funcs.r; adds clean-session containers only.
...
scanpy_pal <- readRDS('../../data/assets/sc_palettes.rds')
...
# Wilcoxon test with explicit Benjamini-Hochberg correction -------------------
# `stat_compare_means(comparisons = ...)` delegates each comparison separately
# to ggsignif, so a p.adjust.method argument does not adjust that comparison
# family.  This helper calculates the complete family first, applies BH within
# each facet, writes the statistical source data, and then adds numeric adjusted
# P values to the plot.
add_bh_wilcox <- function(plot,
                          comparisons,
                          x,
                          y,
                          facet_by = NULL,
                          ...
                          stats_file = NULL,
                          step.increase = 0.12,
                          annotation_y_transform = identity) {
  require(ggpubr)
  require(dplyr)
  require(readr)

  stat_data <- as.data.frame(plot$data) %>%
    dplyr::filter(!is.na(.data[[x]]), !is.na(.data[[y]]))
  stat_data[[x]] <- droplevels(as.factor(stat_data[[x]]))
  if (!is.null(facet_by)) {
    stat_data <- stat_data %>%
      dplyr::filter(!is.na(.data[[facet_by]]))
  }

  # Calculate each requested comparison directly with stats::wilcox.test.
  # tryCatch preserves an NA record for non-estimable all-tie or missing-group
  # comparisons instead of aborting the complete multi-panel figure.
  if (is.null(facet_by)) {
    facet_data <- list(stat_data)
  } else {
    facet_data <- stat_data %>%
      dplyr::group_by(dplyr::across(dplyr::all_of(facet_by))) %>%
      dplyr::group_split(.keep = TRUE)
  }
  stat_res <- lapply(facet_data, function(subdata) {
    facet_values <- if (is.null(facet_by)) {
      NULL
    } else {
      subdata %>%
        dplyr::ungroup() %>%
        dplyr::slice(1) %>%
        dplyr::select(dplyr::all_of(facet_by))
    }
    comparison_res <- lapply(comparisons, function(comp) {
      value1 <- subdata[[y]][as.character(subdata[[x]]) == comp[1]]
      value2 <- subdata[[y]][as.character(subdata[[x]]) == comp[2]]
      value1 <- value1[is.finite(value1)]
      value2 <- value2[is.finite(value2)]
      test <- tryCatch(
      ...
      dplyr::tibble(
        ...
        method = ifelse(paired, 'Wilcoxon signed-rank test', 'Wilcoxon rank-sum test')
      )
    }) %>% dplyr::bind_rows()
    if (!is.null(facet_values)) {
      comparison_res <- dplyr::bind_cols(
        facet_values[rep(1, nrow(comparison_res)), , drop = FALSE],
        comparison_res
      )
    }
    comparison_res
  ...
  # rstatix omits p.adj when a family has only one comparison.  Calculate BH
  # explicitly so every output has the same auditable columns and so grouping
  # is unambiguously within each plotted facet.
  if (!is.null(facet_by)) {
    y_range <- stat_data %>%
      dplyr::group_by(dplyr::across(dplyr::all_of(facet_by))) %>%
      dplyr::summarise(
    ...
    stat_res <- stat_res %>%
      dplyr::left_join(y_range, by = facet_by) %>%
      dplyr::group_by(dplyr::across(dplyr::all_of(facet_by)))
  } else {
    stat_res$.y_max <- max(stat_data[[y]], na.rm = TRUE)
    stat_res$.y_min <- min(stat_data[[y]], na.rm = TRUE)
    stat_res <- stat_res %>% dplyr::group_by(.y.)
  }

  x_levels <- levels(stat_data[[x]])
  stat_res <- stat_res %>%
    dplyr::mutate(
      .y_span = dplyr::if_else(
        is.finite(.y_max - .y_min) & (.y_max - .y_min) > 0,
        .y_max - .y_min,
        pmax(abs(.y_max) * 0.1, 1)
      ),
      .comparison_index = dplyr::row_number(),
      y.position = .y_max + (0.08 + step.increase * (.comparison_index - 1)) * .y_span,
  ...
  stat_res <- stat_res %>%
    ...
    dplyr::mutate(
      p.adj.label = format(signif(p.adj, 3), scientific = TRUE, trim = TRUE),
      p.adjust.method = 'BH',
      alternative = 'two.sided',
      paired = paired
    ) %>%
  ...
  if (!is.null(stats_file)) {
    readr::write_tsv(stat_res, stats_file)
  }

  plot_stat <- stat_res %>%
    dplyr::filter(!is.na(p.adj), !is.na(y.position))
  if (nrow(plot_stat) == 0) {
    return(plot)
  }
  # ggpubr's bracket stat expects annotation positions in transformed panel
  # coordinates for non-linear y scales.  Keep the auditable stats table in
  # original data units, and transform only the copy used for annotation.
  plot_stat$y.position <- annotation_y_transform(plot_stat$y.position)
...
# Export the exact data frame carried by a ggplot/Seurat plot.  Row names are
# retained because they are cell barcodes for Seurat UMAP and DotPlot objects.
write_plot_source_data <- function(plot, filename) {
  source_data <- as.data.frame(plot$data)
  if (!identical(rownames(source_data), as.character(seq_len(nrow(source_data))))) {
    source_data <- tibble::rownames_to_column(source_data, var = 'observation_id')
  }
  readr::write_tsv(source_data, filename)
